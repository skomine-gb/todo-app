...
| ステップ                                               | 内容                                                                           |
| ------------------------------------------------------ | ------------------------------------------------------------------------------ |
| checkout〜`vp install`                                 | `ci` job（[§3](#3-ci-job-がやること)）と同じセットアップ。main の最新を取得    |
...
- **fork からの PR**：GitHub の仕様で Secrets は fork からの PR には渡らない。`ci` job は Secrets を使わないので問題なく動く。Secrets を使う deploy.yml は、トリガーが「main での CI 完了」と手動実行だけなので PR では起動しない
