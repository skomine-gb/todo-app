...
| 10   | `feat/step10-todos-update-delete` | PATCH / DELETE を実装（UPDATE は `RETURNING` を使うと更新後の行が1クエリで取れる）                                                                                                                                                                        | `completed` の反転が保存される／`title` の編集／空文字は `400`／存在しない `id` は `404`／DELETE が `204` で一覧から消える／存在しない `id` の DELETE は `404`                             | [08](./08-api-design.md) §3           |
